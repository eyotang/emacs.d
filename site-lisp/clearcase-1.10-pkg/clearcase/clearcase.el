...
(define-key clearcase-prefix-map "f" 'clearcase-find-checkouts-in-current-view)
...
(defun clearcase-gui-merge-latest-current-buff ()
  "Use GUI to merge a version in the current buffer with its LATEST version."
  (interactive)
  (clearcase-gui-merge-file-with-version buffer-file-name
                                         (clearcase-get-latest-version buffer-file-name))
  ;; Resync.
  ;;
  (clearcase-sync-from-disk buffer-file-name t))
...
(defun clearcase-gui-merge-file-with-version (truename other-version)
(let* ((other-vxpath (clearcase-vxpath-cons-vxpath (clearcase-vxpath-element-part truename)
					     other-version))
       (other-file (if (clearcase-file-is-in-mvfs-p truename)
                       other-vxpath
  ...
  (clearcase-ct-do-cleartool-command "merge"
                                     other-file
                                     'unused
                                     (list "-g" "-to" (clearcase-path-native truename)))))
...
a buffer is popped up to accept one."
  ...
  (if (null comment)
      ;; If no comment supplied, set to unused
      (setq comment 'unused))
  (if (null comment)
      ;; If no comment supplied, go and get one..
      ;;
      ...
      (let ((buffer-on-file (find-buffer-visiting file)))
        (if buffer-on-file
            (progn
              (set-buffer buffer-on-file)
              (clearcase-sync-to-disk))))
      (setq result (clearcase-ct-do-cleartool-command "ci"
                                         file
                                         comment
                                         clearcase-checkin-arguments))
      ;; catch the checkin error, and pop up GUI Merge with LATEST version
      (if (stringp result)
          (progn
            (clearcase-gui-merge-file-with-version file
                                                   (clearcase-get-latest-version file))
            ;; check in once again
...
(defun clearcase-compose-latest-version (version-string)
  (let ((branch (clearcase-vxpath-branch version-string)))
    (format "%sLATEST" branch)))

(defun clearcase-get-latest-version (file)
  "Get the predecessor version first, and compose
the LATEST version manually."
    (clearcase-compose-latest-version 
     (clearcase-fprop-predecessor-version file)))
  ...
  (let ((camefrom (current-buffer))
    ...
    (if (re-search-forward "^cleartool: Error:.*$" nil t)
        (progn
          ...
          (if (string-match "most recent version .* not the predecessor" status) ()
          (clearcase-port-view-buffer-other-window "*clearcase*")
          (shrink-window-if-larger-than-buffer)
...
(defun clearcase-ct-blocking-call (&rest cmd)
  ...
  (if (string-match "cleartool: Error:" clearcase-ct-return)
      (if (string-match "cleartool: Error: Unable to check in" clearcase-ct-return)()
